...
const projectTable = [
  {
    ...
    country: 'USA',
    information: "EliteSingles Unpaid - Age: 42, Gender:MALE - Seeking:FEMALE, Location:Monterey/CA/93942",
    price: "4.00",
    date: '10 may 2023',
  },
...
mock.onGet('/pages/profile-table').reply(config => {
  ...
  const filteredData = projectTable.filter(row => {
    return (
      row.type.toLowerCase().includes(queryLowered) ||
      row.date.toLowerCase().includes(queryLowered) ||
      row.country.toLowerCase().includes(queryLowered)
    )
  })
